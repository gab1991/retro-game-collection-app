...
export const ebayItemsReducer = createReducer<TEbayItemsReducer, TEbayItemsActions>(initial)
  .handleAction(actions.setEbayItems, (state, { payload }) => {
  ...
  .handleAction(actions.setEbaySingleItemData, (state, { payload }) => {
    return produce(state, (draft) => {
  ...
  .handleAction(actions.setIsWatchedEbayCard, (state, { payload }) => {
    return produce(state, (draft) => {
  ...
  .handleAction(actions.setEbayItemShippingLoading, (state, { payload }) => {
    return produce(state, (draft) => {
      const { platform, game, sortOrder, index, bool } = payload;
      if (!game || !platform || (!index && index !== 0)) return;
  ...
  .handleAction(actions.setEbayItemShippingCost, (state, { payload }) => {
    return produce(state, (draft) => {
  ...
  .handleAction(actions.calculateTotalPrice, (state, { payload }) => {
    return produce(state, (draft) => {
  ...
  .handleAction(actions.setContactSeller, (state, { payload }) => {
    return produce(state, (draft) => {
      const { platform, game, sortOrder, index, bool } = payload;
      if (!game || !platform || (!index && index !== 0)) return;
