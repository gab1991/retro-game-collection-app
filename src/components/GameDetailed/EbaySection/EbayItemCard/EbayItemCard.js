...
export default function EbayItemCard(props) {
  const {
    ...
    ShippingCostSummary: shipping,
    ListingType: listingType,
    ViewItemURLForNaturalSearch: itemUrl
  } = props.Item;
  const descParsed = ReactHtmlParser(Description);
  console.log(props.Item);
  const currentPrice = ConvertedCurrentPrice.Value;
  const currency = ConvertedCurrentPrice.CurrencyID;
  const [deliveryPrice, setDelivertPrice] = useState(
    shipping.ShippingServiceCost ? shipping.ShippingServiceCost.Value : ''
  );
  const total = Number(deliveryPrice + currentPrice).toFixed(2);
  ...
  return (
    <div className={styles.EbayItemCard}>
      ...
      <div className={styles.Description}>
        ...
        <Button
          txtContent={
            listingType === 'FixedPriceItem' ? 'Buy It Now' : 'Place bid'
          }
          onClick={sendToEbay}
        />
        <div className={styles.PriceSection}>
          ...
          <p className={styles.Delivery}>
            ...
            {deliveryPrice && `${'+ Shipping'} ${deliveryPrice} ${currency}`}
          </p>
          <hr></hr>
